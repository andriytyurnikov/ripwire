...
TABLE = {
    ...
    ( "src/verbs_report.h", "hdr" ):    ( 1, "safe",       "hdr[768]: writeSkippedHeader's <skipped ...> root for runSkipped (§L1). ~235 B of literal + FOURTEEN integer counters at 20 B worst case = 280 B, + ignore_mode's closed label (12 B, 'root-ignored'), + nestAttr (the bounded buffer rowed above, 36 B worst case), + the compile-time literal ' rows_capped=\"1\"' or '' (18 B) = ~581 B against 767 usable. No path, no name, nothing user-supplied reaches this buffer — every emitted path goes through escapeXml straight into the writer, outside it." ),
...
EXPECTED = { "mentions": 317, "calls": 214, "sites": 214, "rows": 90, "widthforms": 0 }
