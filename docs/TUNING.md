...
| --- | --- | --- | --- | --- | --- |
| 125 | 124 | 112 | 12 | **37** | 75 |

The first two columns are not the same number, and the gap is not a rounding: `src/` holds
**125 cap declarations** under **124 distinct names** (`kRowCap` declared in more than one file). The
sweep patches by NAME, so `112 + 12` accounts for the 124 NAMES — not the 125 declarations. Quoting
"113 of 125" would be wrong in both halves at once, which is exactly the shape of error a
...
finding, and it says what NOT to do: this is not a 125-cap audit. Most of these constants are
