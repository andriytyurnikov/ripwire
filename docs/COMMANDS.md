...
_The directory-as-knowledge-base pattern: --recall pointed at a 2018057-byte scratch dir of DUMPED TOOL OUTPUT (a git log, this repo's own generated command reference, --help text, an architecture doc, a fabricated JSON access log) instead of a source repo — no index to build, no daemon._
...
**Answers:** decl skeletons with the bodies cut — ~73-91% fewer element bytes than the same symbols' full --expand bodies, about 71% at top-50 (the sigs payload is always top-50, whatever --top-k says).
