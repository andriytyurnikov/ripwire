...
than an unmarked one," not as "the whole symbol is now precise." A path that is missing, empty or a directory
refuses (exit 1); a corrupt index warns on stderr and proceeds all-name-based (same stdout as not passing
`--scip`) — check header `precise=N` to confirm the overlay actually matched anything.
